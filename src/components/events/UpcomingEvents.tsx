  ...
  const events = [
    {
      ...
      description: "Stallion's Special Yoga Event on the Yoga Day",
  ...
  return (
    <section className="py-20 bg-white px-2 sm:px-6">
      <div className="container mx-auto px-4">
        ...
        <div className={`grid ${events.length === 1 ? 'grid-cols-1 md:flex md:justify-center' : 'grid-cols-1 md:grid-cols-3'} gap-6`}>
          {events.map((event, index) => (
            <ScrollAnimation key={event.id} delay={0.2 * index}>
              <Link href={event.buttonLink} className="block">
                <HoverCard className={`relative overflow-hidden h-[600px] cursor-pointer ${events.length === 1 ? 'md:w-[400px]' : ''}`}>
                  {/* Event Image */}
